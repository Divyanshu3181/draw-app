...
export class Game {
    ...
    if (this.selectedTool === "pencil") {
      ...
      const shapeToMove = [...this.existingShapes].reverse().find((shape) => {
        if (shape?.type === "rect") {
          ...
          return (
            Math.abs(Math.hypot(this.startX - shape.centerX, this.startY - shape.centerY)) <= shape.radius
          );
        } else if (shape?.type === "pencil") {
  ...
  mouseUpHandler = (e: MouseEvent) => {
    ...
    if (this.selectedTool === "rect") {
      ...
      const radius = Math.abs(Math.hypot(endX - this.startX, endY - this.startY));
      if (radius > 1) {
  ...
  mouseMoveHandler = (e: MouseEvent) => {
    ...
    if (this.selectedTool === "rect") {
      ...
      this.redrawCanvas();
      const radius = Math.abs(Math.hypot(x - this.startX, y - this.startY));
      this.ctx.beginPath();
  ...
  eraseShape(x: number, y: number) {
    ...
    this.existingShapes = this.existingShapes.filter((shape) => {
      if (shape?.type === "rect") {
        ...
        return Math.abs(Math.hypot(x - shape.centerX, y - shape.centerY)) > shape.radius;
      } else if (shape?.type === "pencil") {
